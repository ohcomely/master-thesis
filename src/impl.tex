...
\begin{tikzpicture}[
    ...
    transform/.style={box, fill=orange!15},
...
\node[transform, right=of matrix] (graph) {Graph $G=(V,E)$\\Vertices = rows/cols\\Edges = nonzeros};
\node[process, right=of graph] (cliques) {Edge-Clique\\Cover};
\node[transform, right=of cliques] (cnh) {Clique-Node\\Hypergraph};

% Second row (right to left, below the first row)
\node[process, below=of cnh] (partition) {Partition with\\KaHyPar into $k$};
\node[process, left=of partition] (separator) {Extract\\Separator};
\node[process, left=of separator] (amd) {Apply AMD to\\each block};
\node[result, left=of amd] (permute) {Final\\Permutation};
...
\begin{algorithm}[H]
    \SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
    \SetKwFunction{SortNeighborsByDegree}{SortNeighborsByDegree}
    \SetKwFunction{GPUBFS}{GPU\_BFS}
    ...
    \ForEach{vertex $u \in V$ \textbf{in parallel on GPU}}{
        $\SortNeighborsByDegree(u)$ \tcp{Parallely sort neighbors}
    }
    \BlankLine
    ...
    \ForEach{component start vertex $s$ (by increasing degree)}{
        \If{$s \notin \text{visited}$}{
            $\text{comp\_order} = \GPUBFS(G, s)$ \tcp{BFS from source $s$}
            $\text{visited} = \text{visited} \cup \text{comp\_order}$\;
            $\text{order} = \text{order} + \text{comp\_order}$\;
        }
    }
    \BlankLine
