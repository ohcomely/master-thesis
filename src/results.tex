...
We selected a diverse set of sparse matrices from the SuiteSparse Matrix Collection and our custom dataset of matrices encountered in field-specific domains, developed in Nano-TCAD, to evaluate our implementations. The SuiteSparse matrices are chosen to cover a variety of application domains, including computational fluid dynamics (CFD), chemistry, circuit simulation, graph analysis, optimization, and structural engineering. We have listed the matrices alongside their respective groups, number of vertices, and number of edges in Table~\ref{tab:matrix-dataset}. The Quantum Transport matrices are listed in Table~\ref{tab:qt-matrix-dataset}.
